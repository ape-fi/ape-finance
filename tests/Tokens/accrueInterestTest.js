...
describe('CToken', () => {
  ...
  beforeEach(async () => {
    ...
    cToken2 = await makeCToken({ kind: 'ccollateralcapnointerest' });
  });

  beforeEach(async () => {
    await preAccrue(cToken);
    await preAccrue(cToken2);
  });
  ...
  describe('accrueInterest', () => {
    it('reverts if the interest rate is absurdly high', async () => {
      ...
      await expect(send(cToken, 'accrueInterest')).rejects.toRevert("revert borrow rate too high");
    });
